...
```bash
source <(~/.gemini/skills/gstack/bin/gstack-slug 2>/dev/null) && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
write_file to `~/.gstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With Gemini CLI + gstack,
...
* The 1-year question. read_file this plan as a new engineer in 12 months — obvious?
...
### TODOS.md updates
Present each potential TODO as its own individual ask_user. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.gemini/skills/review/TODOS-format.md`.
...
* **Effort estimate:** S/M/L/XL (human team) → with Gemini+gstack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
~/.gemini/skills/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
After displaying the Review Readiness Dashboard, recommend the next review(s) based on what this CEO review discovered. read_file the dashboard output to see which reviews have already been run and whether they are stale.
